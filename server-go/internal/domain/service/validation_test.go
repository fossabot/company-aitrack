...
// fakeCounter implements port.EditRecordCounter for tests.
...
func defaultValidationSvc(counter port.EditRecordCounter) *service.ValidationService {
	return service.NewValidationService(
		service.NewSignatureService(),
		service.NewDiffConsistencyService(),
		counter,
		service.ValidationPolicy{RateLimitPerHour: 30, MaxAddedLines: 5000},
	)
}
...
func TestStep6_RepoNotInWhitelist_SoftFlag(t *testing.T) {
	policy := service.ValidationPolicy{
		...
		RepoWhitelistURLs: []string{"git@github.com:allowed/repo.git"},
	}
