...
class EditsControllerTest {
    ...
    private String validTokenKey() {
        return TokenService.computeTokenKey(RAW_TOKEN);
    }
    ...
    void tooManyEditsInBatch_413() throws Exception {
        ...
        java.util.List<EditDto> edits = new java.util.ArrayList<>();
        EditDto template = buildSignedEdit(tokenKey);
        for (int i = 0; i < 501; i++) {
            edits.add(template);
        }
        EditBatchRequest req = EditBatchRequestFactory.withEdits(edits);
