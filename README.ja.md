...
| **Go サーバー** `aitrack-server-go` | Go 1.25 · chi v5.2.5 · SQLite / PostgreSQL | Java と機能同等の軽量代替実装 |
...
| **最小限の収集** | ファイルパス、変更 diff（unified diff 形式、変更行のみ）、行数統計、リポジトリのメタデータ、および任意のプロンプト要約（ツール使用分類用）を収集；完全な会話履歴やキーボード入力は収集しない |
